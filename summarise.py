...
def write_perSNP_summary(genes, sampleIDs, count_mask, header, delimiter=',', multi_hits=False, out=sys.stdout):
...
def main(argv):
    ...
    out_header = [('%s:Col,%s:Ped,%s:Total_Reads' % (sid, sid, sid)).split(',')
                  for sid in sampleIDs]
    out_header = [first_header, 'SNP'] + [item for sublist in out_header for item in sublist]
    
    write_perSNP_summary(genes, sampleIDs, count_mask, out_header, delimiter=delimiter, multi_hits=multi_hits,
                         out=open('%sgenic_snp_summary.csv' % (hit_mode), 'wb'))  
    
    if hit_mode == 'intra':
        out_header = [('%s:Col,%s:Ped,%s:Total_Reads,%s:SNPs_covered' % (sid, sid, sid, sid)).split(',')
                      for sid in sampleIDs]
        out_header = [first_header, '#SNPs'] + [item for sublist in out_header for item in sublist]
    #print out_header
    
        write_perGene_summary(genes, sampleIDs, count_mask, out_header, delimiter=delimiter, multi_hits=multi_hits,
                              out=open('%sgenic_gene_summary.csv' % (hit_mode), 'wb'))
